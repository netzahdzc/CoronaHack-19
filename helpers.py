...
def save_images(iteration, fixed_Y, fixed_X, G_YtoX, G_XtoY, batch_size=16, sample_dir='fake_images_cyclegan', option='fake'):
    ...
    if option == 'fake':
        ...
        for idx, (s, t) in enumerate(zip(X, fake_Y)):
            merged = np.zeros([3, h, w])
            i = 0
            j = 0
            merged[:, i*h:(i+1)*h, (j*2)*h:(j*2+1)*h] = t
            merged = merged.transpose(1, 2, 0)
            path = os.path.join(sample_dir, 'X-Y', 'fake-{:06d}-X-Y.png'.format(counter))
#             scipy.misc.imsave(path, merged)
            img_uint8 = merged.astype(np.uint8)
            imageio.imwrite(path, img_uint8)
            print('Fake image saved {}'.format(path))
            counter += 1
    ...
    if option == 'fake':
        ...
        for idx, (s, t) in enumerate(zip(Y, fake_X)):
            ...
            merged[:, i*h:(i+1)*h, (j*2)*h:(j*2+1)*h] = t
            merged = merged.transpose(1, 2, 0)
            path = os.path.join(sample_dir, 'Y-X', 'fake-{:06d}-Y-X.png'.format(counter))
#             scipy.misc.imsave(path, merged)
            img_uint8 = merged.astype(np.uint8)
            imageio.imwrite(path, img_uint8)
            print('Fake image saved {}'.format(path))
